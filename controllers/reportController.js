...
const exportTasksReport = async (req, res) => {
  try {
    const tasks = await Task.find({ createdBy: req.user._id }) // ✅ filter by admin
...
const exportUsersReport = async (req, res) => {
  try {
    const users = await User.find({ adminId: req.user._id }) // ✅ only this admin's members
      .select("name email _id")
      .lean();

    const userTasks = await Task.find({ createdBy: req.user._id }) // ✅ only this admin's tasks
